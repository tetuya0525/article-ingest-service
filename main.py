...
def validate_article_data(data):
    ...
    if not isinstance(data.get("content"), dict) or "rawText" not in data.get(
        "content"
    ):
        return (
            False,
            "contentフィールドは'rawText'を含むオブジェクトである必要があります。",
        )
...
def ingest_article():
    ...
    try:
        ...
        log_structured(
            "ERROR", "記事の投入処理中に予期せぬエラー", error=str(e), exc_info=True
        )
...
def health_check():
    ...
    try:
        ...
        return (
            jsonify({"status": "unhealthy", "error": "Database connection failed"}),
            503,
        )
