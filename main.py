...
# ==============================================================================
# 受付係の魂 (Flask直接起動版)
# これまでのfunctions-frameworkをやめ、より標準的なFlaskでサービスを起動します。
# これにより、フレームワーク固有の問題を完全に切り分けます。
# ==============================================================================
import os
...
def report_error(error_message, context, tb_str):
    ...
    if not LOG_AGGREGATOR_URL:
        ...
        return
    try:
        log_payload = {"serviceName": SERVICE_NAME, "errorMessage": error_message, "context": context, "traceback": tb_str}
...
# リクエストを処理するルートを「/」に明確に設定
@app.route("/", methods=['POST'])
def article_ingest_service():
    """
...
if __name__ == "__main__":
    # この部分はローカルテスト用です。Cloud Runではgunicornが使われます。
